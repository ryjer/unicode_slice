...
int64_t unicode切片_转整数(unicode切片 * 切片指针, uint8_t 进制) {
    ...
    if (进制 == 0) {
        if (切片指针->数据[0]=='0') {       // 自动判断进制，并调整第一个有效数字字符指针 i
            ...
            else if (是10进制数字(切片指针->数据[1])) { i=0; 进制=10; } // 有多余0前缀的10进制数字
        } else {
            i = 0;
            进制 = 10;
        }
...
unicode静态串 * unicode切片_转静态串(unicode切片 * 切片指针) {
    // 申请容纳空间
    unicode静态串 * 新静态串 = (unicode静态串 *)malloc(sizeof(unicode静态串)+(切片指针->长度)*sizeof(unicode字符));
    // 复制数据
    新静态串->长度 = 切片指针->长度;
    for (int i=0; i<切片指针->长度; i++) {
        新静态串->数据[i] = 切片指针->数据[i];
 ...
 * 切片指针：unicode切片/字符串存储结构
 *********************************************************/
unicode动态串 * unicode切片_转动态串(unicode切片 * 切片指针) {
    unicode动态串 * 新动态串 = (unicode动态串 *)malloc(sizeof(unicode动态串)+(切片指针->容量)*sizeof(unicode字符));
    // 复制数据
    新动态串->长度 = 切片指针->长度;
    新动态串->容量 = 切片指针->容量;
    for (int i=0; i<切片指针->长度; i++) {
